...
>  首先是完成对8259中断控制器的初始化和IDT的初始化，8259中断控制器的初始化通过pic_init()来完成，而IDT的初始化，也就是需要CPU建立一个中断描述符表，这个中断描述符表中就包含了对于时钟中断的一个中断门，其中包含了两个信息，一个是段描述符，一个是offset。当时钟中断产生的时候，会有一个中断号作为index去查建立好的IDT描述符表，进而找到对应的中断服务例程的地址；最后一步准备工作是Enable中断，也就是使能中断(intr enable)，还需要对时钟外设做相应的初始化，跟时钟相关的是8253这个芯片的一些设置。
...
>  完成了包括了对串口，并口，CGA也就是我们的显示器以及时钟，就是产生时钟中断，还有硬盘的读取这些外设的访问。
...
int main()
{
  example("example", "12" , "23", "34", "end");
  example("example", "123", "345", "end");
  return 0;
}
