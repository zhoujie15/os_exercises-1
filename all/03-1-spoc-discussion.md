...
```
 ...
 ```
- 最先匹配：优点：实现简单，在高地址空间有大块分区空间。缺点：会存在外部碎片，在分配大块时比较慢，分配速度会越来越慢。
- 最佳匹配：优点：大部分分配的尺寸较小时，效果很好，可以避免大的空闲分区被拆分，减小外部碎片的大小。实现相对简单。缺点：会存在外部碎片。释放分区较慢。会产生难以被利用的小碎片。
- 最差匹配：优点：中等大小的分配较多时，效果很好。能够避免出现太多的小碎片。缺点：释放分区较慢。存在外部碎片。容易破坏大的空闲分区，后续难以分配大的分区。
- buddy system：优点：地址分配整齐，访问起来迅速，外部碎片少。缺点：存在内部碎片，降低了内存利用率。
- 可以使用高级数据结构进行内存分配，比如线段树等等，这样可以提高查找和分配效率。这样的方法比较类似buddy system，但是能够利用空余的空间，比如120字节，在buddy system中需要分配128字节，但是使用线段树的话，配合标记下传，可以正好分配120字节。
